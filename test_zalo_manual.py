...
def test_zalo_manual_message_receipt(
    ...
    try:
        ...
        if resp.status_code != 200:
            raise RuntimeError(
                f"Tunnel health check failed with status {resp.status_code}: {resp.text}"
            )
    ...
    if target_id:
        ...
        resp = request_with_retry(
            "post", f"{BASE_URL}/bots/{bot_id}/send", json=send_payload
        )
        if resp.status_code != 200:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for msg in messages:
                    ...
                    if (
                        msg.get("platform_name") == "Zalo"
                        and msg.get("content") == random_text
                    ):
                        if not group_id or str(msg.get("platform_conv_id")) == str(
                            group_id
                        ):
                            print(
    ...
    assert (
        received
    ), f"Did not receive the expected text '{random_text}' from Zalo within 5 minutes."
