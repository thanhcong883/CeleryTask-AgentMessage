...
async def verify_token(request: Request, expected_token: str):
    ...
    if auth_header:
        ...
        token = (
            alt_auth_header[7:]
            if alt_auth_header.startswith("Bearer ")
            else alt_auth_header
        )
    ...
    if not token and request.method in ["POST", "PUT"]:
        try:
            ...
            if isinstance(body, dict):
                token = (
                    body.get("secretToken")
                    or body.get("secret_token")
                    or body.get("token")
                )
        except Exception:
            pass
    ...
    if not token or token != expected_token:
        logger.warning(
            f"Unauthorized access attempt to {request.url.path}. Headers: {request.headers}"
        )
