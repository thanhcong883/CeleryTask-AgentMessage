...
def config_zalo_webhook(bot_id: str, base_url: str):
    ...
    webhook_url = f"{base_url}/api/hook?platform=zalo&secretToken={config.HOOK_TOKEN}"
    response = requests.post(
        url,
        json={"webhookUrl": webhook_url, "secretToken": config.HOOK_TOKEN},
        timeout=10,
    )
    response.raise_for_status()
    return response.json()
...
def sync_zalo_webhook(bot_id: str, base_url: str):
    ...
    current_webhook = (
        f"{base_url}/api/hook?platform=zalo&secretToken={config.HOOK_TOKEN}"
    )
    try:
        # 1. Check if account exists
        accounts = get_zalo_accounts()
        account_exists = any(
            str(acc.get("accountId")) == str(bot_id) for acc in accounts
        )
