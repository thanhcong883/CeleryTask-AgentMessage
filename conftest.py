...
def tunnel_url(server_process):
    ...
    try:
        requests.post("http://localhost:8000/api/config", headers=headers, json={"BASE_URL": url}, timeout=5)
...
def server_process(test_env):
    ...
    for i in range(max_retries):
        try:
            ...
            response = requests.get("http://localhost:8000/docs", headers=headers, timeout=1)
...
def request_with_retry():
    ...
    def _request(method, url, **kwargs):
        ...
        if "Authentication" not in headers and "Authorization" not in headers and "X-Telegram-Bot-Api-Secret-Token" not in headers:
