...
def tunnel_url(server_process):
    ...
    print(
        ">>> WAITING FOR TUNNEL STABILITY (5 consecutive successful pings to /docs)..."
    )
    success_count = 0
    headers = {"Authentication": f"Bearer {config.SECRET_TOKEN}"}
    for _ in range(60):
        try:
            # Note: /docs might not be protected by global dependency if it's generated by FastAPI
            # but root "/" is, so we check root instead which we know is protected
            resp = requests.get(f"{url}/docs", headers=headers, timeout=5)
...
def server_process(test_env):
    ...
    # Ensure all security tokens and test env vars are present
    env["SECRET_TOKEN"] = config.SECRET_TOKEN
    env["HOOK_TOKEN"] = config.HOOK_TOKEN
    ...
    process = subprocess.Popen(
        cmd, stdout=log_file, stderr=log_file, env=env, preexec_fn=os.setsid
    )
    ...
    for i in range(max_retries):
        try:
            # Check root instead of /docs
            response = requests.get(
                "http://localhost:8000/docs", headers=headers, timeout=1
            )
...
def worker_process(test_env):
    ...
    cmd = [
        ...
        "--loglevel=info",
        "-Q",
        "celery_receive_message,celery_send_message,celery_agent_message",
        "--concurrency=1",
    ]
...
def request_with_retry():
    ...
    def _request(method, url, **kwargs):
        ...
        if (
            ...
            and "X-Telegram-Bot-Api-Secret-Token" not in headers
        ):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code < 500:
                    return response
                print(
                    f">>> Request {method} {url} returned {response.status_code}. Retrying..."
                )
