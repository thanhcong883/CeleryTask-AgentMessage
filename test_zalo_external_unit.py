...
def test_create_and_verify_account(headers):
    ...
    assert response.status_code in [
        200,
        201,
    ], f"Failed to create account: {response.text}"
...
def test_zalo_webhook_config(headers):
    ...
    response = requests.post(
        url, headers=headers, json={"webhookUrl": test_url}, timeout=10
    )
    ...
    assert (
        current_data.get("webhookUrl") == test_url
    ), f"Webhook URL mismatch. Expected {test_url}, got {current_data.get('webhookUrl')}"

    # 4. Restore original URL (optional but good practice)
    if original_url:
        requests.post(
            url, headers=headers, json={"webhookUrl": original_url}, timeout=10
        )
