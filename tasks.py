...
def handle_send_message(data: dict, callback: Optional[Callable] = None) -> None:
    """
...
def check_agent_answer(data: dict) -> None:
    ...
    history = get_message_history(conversation_id, message_id)
    if history is None:
        return
...
def _notify_admins_and_customer(data: dict) -> None:
    ...
    for conversation_id in bot_sent_to:
        ...
        send_message.apply_async(
            args=[admin_payload, admin_payload],
    ...
    send_message.apply_async(
        args=[customer_payload, customer_payload],
        queue="celery_send_message"
    )
...
def process_message(data: dict) -> None:
    ...
    user_role = find_user_role(members, data.get("platform_user_id"))
    if user_role == "admin":
        return
...
def _schedule_agent_check(
    ...
    check_response = check_question(data.get("content"))
    ...
    check_agent_answer.apply_async(
        args=[agent_check_data],
        countdown=int(time_to_use_agent)
    )
    logger.info(f"Scheduled agent check for conversation {conversation_id} in {time_to_use_agent}s")
...
def send_message(data: dict, data_send: Optional[dict] = None) -> None:
    ...
    """
    def on_success_callback(platform: str, message_data: dict, send_result: Any) -> None:
...
def _save_bot_sent_message(update_payload: dict, data_send: dict) -> None:
    ...
    if response:
        logger.info(f"Bot message saved for conversation {data_send.get('platform_conv_id')}")
